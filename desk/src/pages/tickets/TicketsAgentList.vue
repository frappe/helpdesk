...
import momentTimezone from "moment-timezone"; // Consider removing if not needed
...
function get_time_zone(timeSelected: string) {
  const userTimeZone = UserDetails.find(d => d.email === user.email)?.time_zone;
  return userTimeZone
    ? momentTimezone.tz(timeSelected, 'YYYY/MM/DD HH:mm', DefaultTimezone).tz(userTimeZone).format('YYYY/MM/DD HH:mm')
    : timeSelected; // return original timeSelected if userTimeZone is not found
}

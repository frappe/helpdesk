...
const isEditingHolidayList = inject<Ref<boolean>>("isEditingHolidayList");

const createNewHolidayList = () => {
  isEditingHolidayList.value = true;
  nextTick(() => {
    router.push({
      name: "NewBusinessHolidays",
      query: { previousPage: router.currentRoute.value.fullPath },
    });
    resetHolidayData();
  });
};

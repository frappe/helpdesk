<template>
	<div class="px-[16px]">
		<ListManager
			...
			<template #body="{ manager }">
				<div class="flex flex-col">
					<div class="pt-[26px] pb-[8px] flex flex-row justify-between items-center">
						<div class="grow">
							Categories
						</div>
						<div>
							<div class="flex items-center space-x-3">
								...
								<div class="cursor-pointer p-1 hover:bg-gray-200 bg-gray-100 rounded" @click="showNewCategoryDialog = true"><FeatherIcon name="plus" class="h-3 w-3"/></div>
							</div>
						</div>
					</div>
							...
							<div v-for="(category, index) in manager.list" :key="category.name">
								<div>
									<CategoryCard 
										...
										@updated="() => { $refs.categories.manager.reload() }"
										@deleted="() => { 
											if (category.name == selectedCategory) {
												$router.push('/frappedesk/knowledge-base')
											}
		...
		<NewCategoryDialog 
			...
			@category-created="() => { $refs.categories.manager.reload() }"
		/>
	</div>
</template>

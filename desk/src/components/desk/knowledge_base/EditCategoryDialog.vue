        ...
        <template #body-content>
            <div>
                ...
                <div v-if="showCategoryDeleteAction">
                    <div v-if="triggerDeleteWarning" class="flex flex-row items-center space-x-3">
                        <div class="flex flex-col space-y-1 grow">
                            <div class="text-[14px]">Delete Category</div>
                            <p class="text-[12px] text-gray-700">Are you sure you want to delete this category ?</p>
                        </div>
                        <div class="flex flex-row space-x-2">
                            <Button appearance="secondary" @click="() => { triggerDeleteWarning = false }">Cancel</Button>
                            <Button 
                                appearance="danger" 
                                @click="() => { 
                                    $resources.deleteCategory.submit({
                                        category: categoryToEdit.category_name
                                    })
                                    reset()
                                }"
                            >
                    ...
                    <div v-else class="flex flex-row items-center space-x-3">
                        <div class="flex flex-col space-y-1 grow">
                            <div class="text-[14px]">Delete Category</div>
                            <p class="text-[12px] text-gray-700">Permanently delete this category</p>
                        </div>
                        <div>
                            <Button appearance="secondary" @click="() => { triggerDeleteWarning = true }">Delete</Button>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    watch: {
        show(val) {
            if (val) {
                this.inputValues.category_name = this.categoryToEdit.category_name;
    ...
    computed: {
        showCategoryDeleteAction() {
            if (this.$resources.articleCount.loading || this.$resources.categoryCount.loading) {
                return false
            }
            return this.$resources.articleCount.data == 0 && this.$resources.categoryCount.data > 1
        }
    },
    setup(props, context) {

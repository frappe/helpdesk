        ...
        >
            <div v-if="!manager.loading">
                <div v-if="manager.list.length > 0">
                    <div v-for="(policy, index) in manager.list" :key="policy.name">
                        <SlaPolicyListItem :class="index == 0 ? 'mt-[9px] mb-[2px]' : 'my-[2px]'" :policy="policy" @toggle-select="manager.select(policy)" :selected="manager.itemSelected(policy)" />
                    </div>
                </div>
                <div v-else>
                    <div class="grid place-content-center h-48 w-full">
                        <div>
                            ...
                            <div class="text-gray-500 mb-2">No policies found</div>
                        </div>
                    </div>
                </div>
            </div>

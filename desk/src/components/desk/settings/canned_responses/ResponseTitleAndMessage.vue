<template>
  <div>
    <div
      v-if="!editable"
      class="flex flex-col space-y-[16px] rounded-[8px] border p-[32px] shadow-sm"
    >
      <div
        class="prose prose-p:my-1 mb-[10px] border-b pb-[16px] text-[24px] font-semibold"
      >
        {{ title }}
      </div>
      <div
        class="overflow-y-auto"
        style="min-height: calc(100vh - 500px); max-height: calc(100vh - 245px)"
        v-html="message"
      ></div>
    </div>
    <div v-else>

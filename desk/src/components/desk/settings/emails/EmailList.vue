        ...
        >
            <div v-if="!manager.loading">
                <div v-if="manager.list.length > 0">
                    <div v-for="(emailAccount, index) in manager.list" :key="emailAccount.name">
                        <EmailListItem :class="index == 0 ? 'mt-[9px] mb-[2px]' : 'my-[2px]'" :emailAccount="emailAccount" @toggle-select="manager.select(emailAccount)" :selected="manager.itemSelected(emailAccount)" />
                    </div>
                </div>
                <div v-else>

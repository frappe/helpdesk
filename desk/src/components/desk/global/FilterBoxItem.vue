<template>
    <div class="flex space-x-2 items-center">
        <div class="w-2/5">
            ...
            >
                <template v-slot="{ toggleDropdown }">
                    ...
                    >
                        <div class="w-full py-1 px-2 bg-gray-100 rounded flex items-center">
                            <div class="grow">{{ getOption(Object.keys(modelValue[filterIndex])[0]).label }}</div>
        ...
        <div class="w-3/5" v-if="getOption(Object.keys(modelValue[filterIndex])[0])">
            <div v-if="getOption(Object.keys(modelValue[filterIndex])[0]).type == 'combobox'">
                <Combobox v-model="selctedFilterValue">
                    ...
                    >
                        ...
                        <ComboboxOption
                            ...
                            v-for="item in filterItems" :key="item"
                            :value="item"
                        >
                            <li
        ...
        <FeatherIcon v-if="removable" name="x" class="w-4 h-4 cursor-pointer hover:stroke-red-500" @click="removeFilter()"/>
    </div>
</template>
...
export default {
    ...
    computed: {
        filterItems() {
            if (this.options && this.getOption(Object.keys(this.modelValue[this.filterIndex])[0])) {
				let items = this.getOption(Object.keys(this.modelValue[this.filterIndex])[0]).items
				return this.query === ''
					? items
					: items.filter((item) => {
						return item ? item.toLowerCase().includes(this.query.toLowerCase()) : null
					})
			} else {
				return []
			}
    ...
    watch: {
        selctedFilterValue(newValue) {
            this.modelValue[this.filterIndex][Object.keys(this.modelValue[this.filterIndex])[0]] = newValue
        }
    },
    methods: {
        getDropdownOptions() {
			...
			this.options.forEach((item, index) => {
				items.push({
					...
					handler: () => {
						...
						this.modelValue[this.filterIndex][this.options[index].name] = ''
					}
				});
			});
			return items;

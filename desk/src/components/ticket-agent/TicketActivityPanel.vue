  ...
  >
    <template #tab-panel="{ tab }">
      ...
      <div v-else class="flex items-center justify-center flex-col flex-1">
        ...
        <p class="text-xl font-medium text-ink-gray-5">{{ __('Loading...') }}</p>
      </div>
    </template>
  </Tabs>
...
import { ActivityIcon, CommentIcon, EmailIcon, PhoneIcon } from '@/components/icons'
import { useActiveTabManager } from '@/composables/useActiveTabManager'
import { useTelephonyStore } from '@/stores/telephony'
import { ActivitiesSymbol, FeedbackActivity, TabObject, TicketSymbol, TicketTab } from '@/types'
...
const _activities = computed(() => {
  ...
  const callProps = (activities.value.data.calls || []).map((call: any) => ({
    ...
    key: call.creation,
    call_type: call.type,
    content: `${call.caller || 'Unknown'} made a call to ${call.receiver || 'Unknown'}`,
  ...
  const sorted = [...emailProps, ...commentProps, ...historyProps, ...callProps, ...taskProps].sort(
    (a: any, b: any) => new Date(a.creation).getTime() - new Date(b.creation).getTime()
  )

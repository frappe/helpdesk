    ...
    <FileUploader :fileTypes="['image/*']" @success="onSuccess">
      <template #default="{ openFileSelector, uploading }">
        ...
        <div v-if="image" class="group relative size-11.5 shrink-0">
          ...
          <button
            type="button"
            class="absolute -right-2.5 -top-2.5 flex size-4.5 cursor-pointer items-center justify-center rounded-full bg-surface-base text-ink-gray-4 opacity-0 outline outline-black-overlay-50 duration-300 ease-in-out focus:outline-none focus-visible:outline-none hover:bg-surface-gray-2 group-hover:opacity-100 [@media(hover:none)]:opacity-100"

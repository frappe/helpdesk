...
export default {
	...
	resources: {
		dataType() {
			return {
				method: "helpdesk.api.general.get_field_data_type",
				params: {
		...
		selectOptionsForField() {
			return {
				method: "helpdesk.api.general.get_select_options_for_field",
			}
		},
	},
	methods: {
		...
		getResourceOptions(filter) {
			switch (filter.data_type) {
				...
				case "Select":
					return {
						method: "helpdesk.api.general.get_filtered_select_field_options",
						inputMap: (query) => {
							return {
								doctype: this.manager.options.doctype,

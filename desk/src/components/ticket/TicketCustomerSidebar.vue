  ...
  <div class="flex w-[382px] flex-col border-l gap-4">
    ...
    <div class="flex flex-col gap-4 pt-0 px-5 py-3 border-b">
      ...
      >
        ...
        <div class="break-words text-base text-gray-800 flex items-center gap-2">
          <Tooltip :text="dayjs(data.value).long()">
            <Badge :label="data.label" :theme="data.theme" variant="subtle" />
          </Tooltip>
          <!-- SLA explanation icon -->
          <Tooltip
              v-if="dayjs(data.value).diff(dayjs(), 'day', true) > 4"
              :text="
                __('This date is calculated based on configured SLAs, working hours, and holidays.')
              "
            >

 ...
 *
 *  The desk resolves this through `inject("listViewData"/"listViewActions")`, which a
 *  Studio block cannot provide — a page script returns bindings, it has no component of
 *  its own to `provide` from. So the host passes the same three things as plain refs and
 *  a callback, and everything below is unchanged. */
export interface FilterSource {
  ...
  /** The current conditions in Frappe wire form. */
  conditions: Ref<FilterCondition[]> | ComputedRef<FilterCondition[]>;
...
export function useFilter(source: FilterSource): Filter {
  const conditions = computed<FilterCondition[]>(() =>
    normalizeFilters(source.conditions.value)
  );

  const activeFilters = computed<ActiveFilter[]>(() => {
    const available = source.fields.value || [];
  ...
  return {
    fields: computed<FilterField[]>(() => source.fields.value || []),
    activeFilters,
    addFilter,
    updateFilter,
    removeFilter,
...
export function normalizeFilters(raw: any): FilterCondition[] {
  ...
  return Object.entries(raw).map(([fieldname, value]): FilterCondition => {
    if (Array.isArray(value)) return [fieldname, value[0], value[1]];

...
- [x] Patch to sync both of the users master
...
- [x] Add a button in Customer.vue in frontend 'Sync with ERPNext' to sync with ERPNext Customer by checking if Integration is enabled or not and if erpnext is installed or not and if both have the same count or not
...
- [x] Add a button in Customer doctype in ERPNext 'Sync with HD' to sync with HD Customer by checking if Integration is enabled or not and if helpdesk is installed or not and if both have the same count or not via Custom List and all scripts, which will come from Helpdesk side
- [x] Desk UI HD Customer hide field
- [x] Settings in HD to toggle integration

Tasks Left
- [ ] Test cases to cover all the above scenarios
...
class TestServiceLevelAgreement(unittest.TestCase):
	...
	def test_service_level_agreement(self):
		...
		self.assertEqual(
			create_customer_service_level_agreement.name,
			get_customer_service_level_agreement.name,
		)
		self.assertEqual(
		...
		self.assertEqual(
			create_customer_service_level_agreement.entity,
			get_customer_service_level_agreement.entity,
		)
		self.assertEqual(
		...
		self.assertEqual(
			create_territory_service_level_agreement.name,
			get_territory_service_level_agreement.name,
		)
		self.assertEqual(
		...
		self.assertEqual(
			create_territory_service_level_agreement.entity,
			get_territory_service_level_agreement.entity,
		)
		self.assertEqual(
	...
	def test_custom_field_creation_for_sla_on_standard_dt(self):
		...
		default_sla = get_service_level_agreement(
			default_service_level_agreement=1, doctype=doctype
		)
		...
		for field in sla_fields:
			self.assertTrue(
				frappe.db.exists(
					"Custom Field", {"dt": doctype, "fieldname": field.get("fieldname")}
				)
			)
	...
	def test_docfield_creation_for_sla_on_custom_dt(self):
		...
		for field in sla_fields:
			self.assertTrue(
				frappe.db.exists(
					"DocField", {"fieldname": field.get("fieldname"), "parent": doctype.name}
				)
			)
...
def create_customer_group():
	...
	if not frappe.db.exists(
		"Customer Group", {"customer_group_name": "_Test SLA Customer Group"}
	):
...
def create_territory():
	territory = frappe.get_doc(
		{"doctype": "Territory", "territory_name": "_Test SLA Territory",}
	)

...
def get_list_data(
    ...
    default_filters = frappe._dict(default_filters or {})
    rows = frappe.parse_json(rows or "[]")
    columns = frappe.parse_json(columns or "[]")
    ...
    default_rows = []
    if hasattr(_list, "default_list_data"):
        default_rows = _list.default_list_data().get("rows") or []
    ...
    # Add default filters if filters are empty
    if default_filters and not filters:
        filters.update(default_filters)

    # Make sure rows contain all column keys
    for column in columns:
        if column.get("key") not in rows:
            rows.append(column.get("key"))
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(), f"Error fetching {doctype} list data")
        return {"error": str(e), "data": [], "columns": columns, "rows": rows}

    # Fetch fields metadata
    fields = frappe.get_meta(doctype).fields
    fields = [f for f in fields if f.fieldtype not in no_value_fields]
    fields = [{"label": f.label, "type": f.fieldtype, "value": f.fieldname, "options": f.options} for f in fields if f.fieldname]
    ...
    std_fields = [
        ...
        {"label": "Created On", "type": "Datetime", "value": "creation"},
        {"label": "Last Modified", "type": "Datetime", "value": "modified"},
        {"label": "Modified By", "type": "Link", "value": "modified_by", "options": "User"},
        {"label": "Assigned To", "type": "Text", "value": "_assign"},
        {"label": "Owner", "type": "Link", "value": "owner", "options": "User"},
    ]
    for f in std_fields:
        if f["value"] not in rows:
            rows.append(f["value"])
        if f not in fields:
            fields.append(f)

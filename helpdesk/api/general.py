...
def get_preset_filters(doctype):
	...
	helpdesk_preset_filter = frappe.qb.DocType("Helpdesk Preset Filter")
	preset_filters = list(
		(
			frappe.qb.from_(helpdesk_preset_filter)
			.select(helpdesk_preset_filter.name)
			.where(
				(
					(
						(helpdesk_preset_filter.type == "User")
						& (helpdesk_preset_filter.user == frappe.session.user)
					)
					| (helpdesk_preset_filter.type.isin(["Global", "System"]))
				)
				& (helpdesk_preset_filter.reference_doctype == doctype)
			)
			.orderby(helpdesk_preset_filter.type)
			.orderby(helpdesk_preset_filter.modified, order=Order.desc)
		).run()
	)

	for preset_filter in preset_filters:
		preset_filter_doc = frappe.get_doc("Helpdesk Preset Filter", preset_filter[0])
		options[
...
def save_filter_preset(doctype, is_global, title, filters):
	...
	helpdesk_preset_filter_items = []
	for filter in filters:
		print(f"label : {filter['label']}")
		helpdesk_preset_filter_items.append(
			{
				"doctype": "Helpdesk Preset Filter Item",
				"label": filter["label"],
	...
	preset_filter = frappe.get_doc(
		{
			...
			"title": title,
			"filters": helpdesk_preset_filter_items,
		}
	)

...
class TestAgentHome(IntegrationTestCase):
    ...
    def test_get_pending_tickets_all_combines_types(self):
        """Test that 'all' type combines different ticket types without duplicates"""
        # Create a second agent for non-matching tickets
        other_agent_email = "other_agent_combined@test.com"
        make_agent(other_agent_email, "Other Agent Combined")

        # Create tickets of different types for current agent
        new_ticket1 = self._create_ticket(subject="New Test Ticket 1")
        new_ticket2 = self._create_ticket(subject="New Test Ticket 2")

        pending_ticket = self._create_ticket(subject="Pending Response Ticket")
        frappe.get_doc(
            {
                "doctype": "Communication",
                "communication_type": "Communication",
                "reference_doctype": "HD Ticket",
                "reference_name": pending_ticket.name,
                "sent_or_received": "Received",
                "content": "Customer message",
            }
        ).insert(ignore_permissions=True)

        # Create tickets for OTHER agent (should NOT be returned)
        self._create_ticket(agent=other_agent_email, subject="Other Agent New Ticket")
        other_pending = self._create_ticket(
            agent=other_agent_email, subject="Other Agent Pending Ticket"
        )
        frappe.get_doc(
            {
                ...
                "communication_type": "Communication",
                "reference_doctype": "HD Ticket",
                "reference_name": other_pending.name,
                "sent_or_received": "Received",
                "content": "Other agent customer message",
            }
        ).insert(ignore_permissions=True)

        result = get_pending_tickets(ticket_type="all")

        # Get ticket names from results
        ticket_names = [t["name"] for t in result["tickets"]]

        # Verify no duplicates
        self.assertEqual(len(ticket_names), len(set(ticket_names)))

        # Verify only current agent's tickets are returned
        self.assertIn(new_ticket1.name, ticket_names)
        self.assertIn(new_ticket2.name, ticket_names)
        self.assertIn(pending_ticket.name, ticket_names)

        # Verify total count is correct for current agent
        self.assertGreaterEqual(result["total_pending_tickets"], 3)

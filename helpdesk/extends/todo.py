...
def validate_team_assignment(doc, event=None):
    ...
    # Exempt the team's own assignment rule, but not custom rules
    if frappe.flags.in_assignment_rule and team:
        if frappe.flags.in_assignment_rule == frappe.db.get_value("HD Team", team, "assignment_rule"):
            return

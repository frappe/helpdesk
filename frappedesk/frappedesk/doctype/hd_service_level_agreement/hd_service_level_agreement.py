...
class HDServiceLevelAgreement(Document):
	...
	def validate_doc(self):
		...
		if self.default_sla and frappe.db.exists(
			...
			frappe.throw(_("Default HD Service Level Agreement for {0} already exists.").format(self.document_type))
	...
	def on_trash(self):
		if self.service_level == "Default":
			frappe.throw("The Default HD Service Level Agreement cannot be deleted")
...
def check_agreement_status():
	...
	for hd_service_level_agreement in hd_service_level_agreements:
		doc = frappe.get_doc("HD Service Level Agreement", hd_service_level_agreement.name)
		if doc.end_date and getdate(doc.end_date) < getdate(frappe.utils.getdate()):
			frappe.db.set_value("HD Service Level Agreement", hd_service_level_agreement.name, "enabled", 0)
...
def get_hd_service_level_agreement_filters(doctype, name):
	...
	return {
		...
		"hd_service_level_agreements": [
			d.name for d in frappe.get_all("HD Service Level Agreement", filters=filters, or_filters=or_filters)
		],
	}
...
def get_documents_with_active_hd_service_level_agreement():
	hd_service_level_agreement_doctypes = frappe.cache().hget("hd_service_level_agreement", "active")
...
def set_documents_with_active_hd_service_level_agreement():
	active = [hd_service_level_agreement.document_type for hd_service_level_agreement in frappe.get_all("HD Service Level Agreement", fields=["document_type"])]
	frappe.cache().hset("hd_service_level_agreement", "active", active)
	return active
...
def change_hd_service_level_agreement_and_priority(self):
	if (
		...
		if not self.hd_service_level_agreement == frappe.db.get_value("HD Ticket", self.name, "hd_service_level_agreement"):
			self.set_response_and_resolution_time(priority=self.priority, hd_service_level_agreement=self.hd_service_level_agreement)
			frappe.msgprint(_("HD Service Level Agreement has been changed to {0}.").format(self.hd_service_level_agreement))


def get_response_and_resolution_duration(doc):
	hd_service_level_agreement = frappe.get_doc("HD Service Level Agreement", doc.hd_service_level_agreement)
...
def get_hd_service_level_agreement_doctypes():
	doctypes = []
	data = frappe.get_list("HD Service Level Agreement", {"enabled": 1}, ["document_type"], distinct=1)
